...
export async function aiDestinationChecker(env: Env, bodyText: string) {
	...
	const result = await generateText({
		...
		prompt:
			...
			Provide a clear reason supporting your determination based on the text and respond with only a JSON object as follows:
			
			{
				"pageStatus": {
					"status": "AVAILABLE_PRODUCT" | "NOT_AVAILABLE_PRODUCT" | "UNKNOWN_STATUS",
					"statusReason": "A concise explanation citing specific words, phrases, or patterns from the content that led to this status. If status is UNKNOWN_STATUS, explain what was missing or ambiguous."
				}
			}
	...
	const validatedResult = schema.safeParse(jsonResult);
